...
class PIDNode(Node):
    def __init__(self):
        ...
        # PID Controllers (linear and angular)
        self.linear_pid = PIDController(kp=0.5, ki=0.0, kd=0.1)
        self.angular_pid = PIDController(kp=1.5, ki=0.0, kd=0.1)
        
        # Target Position (example: x=2.0, y=2.0)
        # You can change these to navigate to different points
        self.target_x = 2.0
        self.target_y = 2.0
    ...
    def control_loop(self):
        ...
        if distance_error < 0.1:
            ...
            # Only move forward if facing the target relatively well
            if abs(angle_error) > 0.2:
                cmd.linear.x = 0.0
            else:
                # We pass distance_error as the setpoint and 0 as current (or vice-versa)
                cmd.linear.x = self.linear_pid.compute(distance_error, 0.0, dt)
                
            # Limit speeds to safe values
